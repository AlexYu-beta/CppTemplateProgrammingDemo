...
#ifndef DEMO1_13_POOL_ALLOCATOR_H
#define DEMO1_13_POOL_ALLOCATOR_H
#pragma once
...
namespace mylib{
    ...
    public:
        ...
        void *base_allocate(size_t n) throw(std::bad_alloc){
            ...
            else if(n>max_size) return static_cast<void*>(operator new(n));
            else return pool[(n+align-1)/align-1].allocate();
        }

        void base_deallocate(void *ptr,size_t n){
            if(n>max_size) operator delete(ptr);
            else pool[(n+align-1)/align-1].deallocate(ptr);
        }
    ...
    public:
        ...
        pointer allocate(size_type n,const void* m=0)throw(std::bad_alloc){
...
#endif //DEMO1_13_POOL_ALLOCATOR_H
